...
// Buscar categoria do balancete pelo slug
$stmt = $pdo->prepare("SELECT * FROM categorias_balancetes WHERE slug = ? AND ativo = 1");
$stmt->execute([$slug]);
...
// Config de cores e ícones por categoria
$config = [
    1 => ['cor' => 'from-red-500 to-red-700', 'icone' => '📊'],
    2 => ['cor' => 'from-green-500 to-green-700', 'icone' => '💰'],
    3 => ['cor' => 'from-blue-500 to-blue-700', 'icone' => '📈']
];
$cfg = $config[$categoria['id']] ?? ['cor' => 'from-blue-600 to-blue-800', 'icone' => '📄'];

// Buscar balancetes desta categoria agrupados por ano
$stmt = $pdo->prepare("
...
$totalDocs = count($balancetes);
$totalAnos = count($porAno);
?>
<!DOCTYPE html>
<html lang="pt-BR">
<head>
    ...
    <div class="max-w-7xl mx-auto px-4">
        <div class="bg-gradient-to-br <?php echo $cfg['cor']; ?> rounded-3xl p-10 shadow-xl relative overflow-hidden">
            ...
            <div class="relative z-10">
                ...
                <h1 class="text-white text-2xl md:text-3xl font-bold border-l-4 border-white pl-4"><?php echo strtoupper($categoria['nome']); ?></h1>
            </div>
        </div>
    </div>
</section>
...
<section class="py-6 bg-gray-200">
    <div class="max-w-7xl mx-auto px-4">
        ...
        <?php else: ?>
            ...
            <!-- Anos com Accordion -->
            <div class="space-y-4">
                <?php $first = true; foreach ($porAno as $ano => $balancetesAno): ?>
