...
$stmt = $pdo->prepare("SELECT * FROM acesso_rapido_cards WHERE slug = ? AND ativo = 1");
$stmt->execute([$slug]);
$card = $stmt->fetch();
...
if ($card['tipo'] === 'link' && !empty($card['link_externo'])) {
    header('Location: ' . $card['link_externo']);
    exit;
}

// Se for PDF direto, redirecionar
if ($card['tipo'] === 'pdf' && !empty($card['arquivo_pdf'])) {
    header('Location: ' . $card['arquivo_pdf']);
    exit;
}
...
foreach ($secoes as $secao) {
    ...
    foreach ($itens as $item) {
        if ($item['tipo'] === 'subsecao') {
            ...
            $item['anos'] = [];
            foreach ($anos as $ano) {
                $stmt2 = $pdo->prepare("SELECT * FROM acesso_rapido_arquivos WHERE ano_id = ? ORDER BY id ASC");
