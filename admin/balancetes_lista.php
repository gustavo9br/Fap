...
// Buscar categoria
$stmtCat = $pdo->prepare("SELECT * FROM categorias_balancetes WHERE id = ?");
...
$cfg = $config[$categoria_id] ?? ['icone' => '📄', 'cor' => 'gray'];
...
if (isset($_GET['deletar']) && is_numeric($_GET['deletar'])) {
    ...
    $stmt = $pdo->prepare("SELECT arquivo FROM balancetes WHERE id = ?");
    $stmt->execute([$id]);
    $bal = $stmt->fetch();
    
    if ($bal && file_exists('../' . $bal['arquivo'])) {
        unlink('../' . $bal['arquivo']);
    }
    
    $stmt = $pdo->prepare("DELETE FROM balancetes WHERE id = ?");
    if ($stmt->execute([$id])) {
        $mensagem = 'Balancete deletado com sucesso!';
        $tipoMensagem = 'success';
    } else {
        $mensagem = 'Erro ao deletar balancete.';
        $tipoMensagem = 'error';
    }
}
...
if (isset($_GET['toggle']) && is_numeric($_GET['toggle'])) {
    $id = $_GET['toggle'];
    $stmt = $pdo->prepare("UPDATE balancetes SET ativo = NOT ativo WHERE id = ?");
    if ($stmt->execute([$id])) {
...
// Buscar balancetes desta categoria
$busca = $_GET['busca'] ?? '';
$ano = $_GET['ano'] ?? '';

$sql = "SELECT * FROM balancetes WHERE categoria_id = ?";
$params = [$categoria_id];
...
$stmtAnos = $pdo->prepare("SELECT DISTINCT ano FROM balancetes WHERE categoria_id = ? ORDER BY ano DESC");
    ...
    <div class="container mx-auto px-4 py-8">
        ...
        <nav class="mb-6">
            <ol class="flex items-center gap-2 text-sm text-gray-600">
                <li>
                    <a href="balancetes_index.php" class="hover:text-blue-600 flex items-center gap-1">
        ...
        <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-4 mb-6">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-2xl md:text-3xl font-bold text-gray-800"><?php echo htmlspecialchars($categoria['nome']); ?></h1>
                    <p class="text-gray-500 text-sm"><?php echo count($balancetes); ?> documento(s)</p>
                </div>
            </div>
            <a href="balancete_form.php?categoria=<?php echo $categoria_id; ?>" 
                            ...
                            <tr>
                                <td colspan="6" class="px-6 py-12 text-center text-gray-500">
                                    <div class="flex flex-col items-center">
                                        ...
                                        Nenhum balancete encontrado nesta categoria.
                                    </div>
                                </td>
                            </tr>
                        <?php else: ?>
                            <?php foreach ($balancetes as $bal): ?>
                                <tr class="hover:bg-gray-50">
                                    <td class="px-6 py-4">
                                        <div class="font-medium text-gray-900"><?php echo htmlspecialchars($bal['titulo']); ?></div>
                                    </td>
                                    <td class="px-6 py-4 text-center text-sm text-gray-600">
                                        <?php echo $bal['data_documento'] ? date('d/m/Y', strtotime($bal['data_documento'])) : '-'; ?>
                                    </td>
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center justify-center gap-2">
                                            <a href="balancete_form.php?id=<?php echo $bal['id']; ?>" 
                                            ...
                                            <a href="?categoria=<?php echo $categoria_id; ?>&toggle=<?php echo $bal['id']; ?>" 
                                               class="text-yellow-600 hover:text-yellow-800 p-1 rounded hover:bg-yellow-50"
                                               title="<?php echo $bal['ativo'] ? 'Desativar' : 'Ativar'; ?>"
                                               onclick="return confirm('Deseja alterar o status?')">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/>
                                                </svg>
                                            </a>
                                            <a href="?categoria=<?php echo $categoria_id; ?>&deletar=<?php echo $bal['id']; ?>" 
        ...
        <div class="mt-6 text-gray-600 text-sm">
            Total de <?php echo count($balancetes); ?> balancete(s) encontrado(s).
        </div>
    </div>
</body>
</html>
