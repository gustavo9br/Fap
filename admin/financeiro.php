...
$icones_disponiveis = [
    ...
    'money' => ['svg' => '<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>', 'nome' => 'Dinheiro'],
    ...
    'calculator' => ['svg' => '<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 14h.01M12 14h.01M15 11h.01M12 11h.01M9 11h.01M7 21h10a2 2 0 002-2V5a2 2 0 00-2-2H7a2 2 0 00-2 2v14a2 2 0 002 2z"/></svg>', 'nome' => 'Calculadora'],
    ...
    'archive' => ['svg' => '<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9 4h4"/></svg>', 'nome' => 'Arquivo'],
    'credit-card' => ['svg' => '<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"/></svg>', 'nome' => 'Cartão'],
    'receipt' => ['svg' => '<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 14l6-6m-5.5.5h.01m4.99 5h.01M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16l3.5-2 3.5 2 3.5-2 3.5 2z"/></svg>', 'nome' => 'Recibo'],
    'trending-up' => ['svg' => '<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6"/></svg>', 'nome' => 'Tendência'],
    'pie-chart' => ['svg' => '<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 3.055A9.001 9.001 0 1020.945 13H11V3.055z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20.488 9H15V3.512A9.025 9.025 0 0120.488 9z"/></svg>', 'nome' => 'Pizza'],
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    if ($_POST['action'] === 'add_card') {
        ...
        // Buscar maior ordem
        $stmt = $pdo->query("SELECT COALESCE(MAX(ordem), 0) + 1 as nova_ordem FROM financeiro_cards");
        ...
        if ($tipo === 'link') {
            // Link interno - cria página dentro de financeiro.php
            $link_externo = '/financeiro/' . $slug;
            $tipo = 'pagina'; // Muda para pagina para permitir edição
        } elseif ($tipo === 'link_externo') {
            ...
            if (move_uploaded_file($_FILES['arquivo_pdf']['tmp_name'], $file_path)) {
                $arquivo_pdf = '/uploads/financeiro/' . $file_name;
            }
        }
        
        $icone_svg = $icones_disponiveis[$icone]['svg'] ?? $icones_disponiveis['clipboard']['svg'];
        
        $stmt = $pdo->prepare("INSERT INTO financeiro_cards (titulo, icone, tipo, link_externo, arquivo_pdf, slug, ordem) VALUES (?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([$titulo, $icone_svg, $tipo, $link_externo, $arquivo_pdf, $slug, $nova_ordem]);
        
        header('Location: financeiro.php?success=added');
        exit;
    }
    
    if ($_POST['action'] === 'update_titulo') {
        $stmt = $pdo->prepare("UPDATE financeiro_cards SET titulo = ? WHERE id = ?");
    ...
    if ($_POST['action'] === 'update_icone') {
        $icone_svg = $icones_disponiveis[$_POST['icone']]['svg'] ?? null;
        if ($icone_svg) {
            $stmt = $pdo->prepare("UPDATE financeiro_cards SET icone = ? WHERE id = ?");
            $stmt->execute([$icone_svg, $_POST['card_id']]);
        }
        header('Location: financeiro.php');
        exit;
    }
    ...
    if ($_POST['action'] === 'delete_card') {
        $stmt = $pdo->prepare("DELETE FROM financeiro_cards WHERE id = ?");
    ...
    if ($_POST['action'] === 'toggle_ativo') {
        $stmt = $pdo->prepare("UPDATE financeiro_cards SET ativo = NOT ativo WHERE id = ?");
        $stmt->execute([$_POST['card_id']]);
        header('Location: financeiro.php');
        exit;
    }
    ...
    if ($_POST['action'] === 'move_card') {
        ...
        if ($card) {
            if ($direcao === 'up') {
                $stmt = $pdo->prepare("SELECT * FROM financeiro_cards WHERE ordem < ? ORDER BY ordem DESC LIMIT 1");
                $stmt->execute([$card['ordem']]);
            } else {
                $stmt = $pdo->prepare("SELECT * FROM financeiro_cards WHERE ordem > ? ORDER BY ordem ASC LIMIT 1");
                $stmt->execute([$card['ordem']]);
            }
            $outro = $stmt->fetch();
            
            if ($outro) {
                $stmt = $pdo->prepare("UPDATE financeiro_cards SET ordem = ? WHERE id = ?");
...
// Buscar cards
$stmt = $pdo->query("SELECT * FROM financeiro_cards ORDER BY ordem ASC");
$cards = $stmt->fetchAll();
...
<div class="max-w-6xl mx-auto space-y-8">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold text-gray-800">💰 Financeiro</h1>
            <p class="text-gray-500 mt-1">Gerencie os cards da seção Demonstrativos Contábeis</p>
        </div>
    </div>
            ...
            <div class="flex items-center justify-between bg-gray-50 p-4 rounded-lg border <?= $card['ativo'] ? 'border-gray-200' : 'border-red-300 bg-red-50' ?>">
                <div class="flex items-center gap-4">
                    ...
                    <div class="relative group">
                        <div class="text-green-600 w-10 h-10 flex-shrink-0 flex items-center justify-center cursor-pointer hover:bg-green-100 rounded-lg" onclick="toggleIconeDropdown(<?= $card['id'] ?>)">
                            <?= $card['icone'] ?>
                        </div>
                        <div id="icone-dropdown-<?= $card['id'] ?>" class="hidden absolute top-full left-0 mt-1 bg-white border rounded-lg shadow-lg p-2 z-50 w-64">
                            ...
                            <div class="grid grid-cols-5 gap-1">
                                ...
                                <form method="POST" class="inline">
                                    ...
                                    <button type="submit" class="p-2 hover:bg-gray-100 rounded text-green-600" title="<?= $icone['nome'] ?>">
                                        <?= $icone['svg'] ?>
                                    </button>
                                </form>
                                <?php endforeach; ?>
                    ...
                    <div class="flex-1">
                        ...
                        <div class="flex items-center gap-2 mt-1">
                            ...
                            <span class="text-xs text-gray-400">→ <?= htmlspecialchars($card['link_externo'] ?: $card['arquivo_pdf'] ?: '/financeiro/'.$card['slug']) ?></span>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-1">
                    ...
                    <a href="editar-financeiro.php?id=<?= $card['id'] ?>" class="p-2 text-purple-500 hover:text-purple-700 hover:bg-purple-50 rounded" title="Editar Conteúdo">
    ...
    <div class="bg-white rounded-xl shadow-md p-6">
        ...
        <form method="POST" enctype="multipart/form-data" class="space-y-4">
            ...
            <div>
                ...
                <div class="flex flex-wrap gap-4">
                    <label class="flex items-center gap-2 p-3 border rounded-lg cursor-pointer hover:bg-gray-50 has-[:checked]:bg-blue-50 has-[:checked]:border-blue-500">
                        ...
                        <span class="text-xs text-gray-400">(/financeiro/slug-do-card)</span>
                    </label>
                    <label class="flex items-center gap-2 p-3 border rounded-lg cursor-pointer hover:bg-gray-50 has-[:checked]:bg-blue-50 has-[:checked]:border-blue-500">
            ...
            <div id="campo_link_externo" style="display:none;">
                ...
                <input type="text" name="link_externo" placeholder="https://exemplo.com ou /balancete/slug" class="w-full border border-gray-300 rounded-lg px-4 py-2">
            </div>
            ...
            <div id="info_link" class="bg-blue-50 border border-blue-200 rounded-lg p-3 text-sm text-blue-700">
                ℹ️ <strong>Página Interna:</strong> Será criada uma nova página em <code>/financeiro/titulo-do-card</code> onde você poderá adicionar seções, anos e arquivos.
            </div>
