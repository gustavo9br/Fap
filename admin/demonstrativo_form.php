...
// Buscar demonstrativo para edição
if ($id && is_numeric($id)) {
    $stmt = $pdo->prepare("SELECT * FROM demonstrativos WHERE id = ?");
    $stmt->execute([$id]);
    $demonstrativo = $stmt->fetch();
    
    if (!$demonstrativo) {
        header('Location: demonstrativos_index.php');
        exit;
    }
    // Se editando, usar categoria do demonstrativo para redirect
    $categoria_preselect = $demonstrativo['categoria_id'];
}
    ...
    if (empty($erros)) {
        try {
            if ($id) {
                ...
                $mensagem = 'Demonstrativo atualizado com sucesso!';
            } else {
                // Inserir
                $stmt = $pdo->prepare("
                ...
                $mensagem = 'Demonstrativo criado com sucesso!';
            }
            
            $tipoMensagem = 'success';
            
            // Redirecionar para a lista da categoria
            $redirect_url = $categoria_preselect ? "demonstrativos_lista.php?categoria=$categoria_preselect" : "demonstrativos_index.php";
...
<head>
    ...
    <title><?php echo $id ? 'Editar' : 'Novo'; ?> Demonstrativo - Admin FAP</title>
...
<body class="bg-gray-50">
    ...
    <div class="container mx-auto px-4 py-8">
        <div class="mb-6">
            <a href="<?php echo $categoria_preselect ? "demonstrativos_lista.php?categoria=$categoria_preselect" : "demonstrativos_index.php"; ?>" class="text-blue-primary hover:text-blue-800 font-medium">
        ...
        <div class="bg-white rounded-lg shadow-md p-8">
            <h1 class="text-3xl font-bold text-gray-800 mb-8">
                <?php echo $id ? 'Editar' : 'Novo'; ?> Demonstrativo
            </h1>
            ...
            <form method="POST" enctype="multipart/form-data" class="space-y-6">
                <?php if ($id): ?>
                    <input type="hidden" name="arquivo_atual" value="<?php echo htmlspecialchars($demonstrativo['arquivo']); ?>">
                <?php endif; ?>
                    ...
                    <select name="categoria_id" 
                        ...
                        <?php foreach ($categorias as $cat): ?>
                            <option value="<?php echo $cat['id']; ?>" 
                                    <?php echo (($demonstrativo && $demonstrativo['categoria_id'] == $cat['id']) || ($categoria_preselect == $cat['id'])) ? 'selected' : ''; ?>>
                ...
                <div>
                    ...
                    <input type="text" 
                           name="titulo" 
                           id="titulo" 
                           value="<?php echo $demonstrativo ? htmlspecialchars($demonstrativo['titulo']) : ''; ?>"
                           required
                           placeholder="Ex: Demonstrativo de Janeiro/2024"
                           class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-primary">
                </div>
                ...
                <div>
                    ...
                    <?php if ($demonstrativo && $demonstrativo['arquivo']): ?>
                        <div class="mb-3 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                            <div class="flex items-center justify-between">
                                <div class="flex items-center gap-2">
                                    ...
                                    <a href="../<?php echo htmlspecialchars($demonstrativo['arquivo']); ?>" 
                                       target="_blank" 
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="date" 
                               name="data_documento" 
                               id="data_documento" 
                               value="<?php echo $demonstrativo ? $demonstrativo['data_documento'] : ''; ?>"
                               required
                    ...
                    <div>
                        ...
                        <input type="number" 
                               name="ano" 
                               id="ano" 
                               value="<?php echo $demonstrativo ? $demonstrativo['ano'] : date('Y'); ?>"
                               required
                               min="1900"
                               max="2100"
                ...
                <div>
                    ...
                    <input type="number" 
                           name="ordem" 
                           id="ordem" 
                           value="<?php echo $demonstrativo ? $demonstrativo['ordem'] : 0; ?>"
                           min="0"
                           class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-primary">
                    <p class="mt-2 text-sm text-gray-600">
                ...
                <div class="flex items-center">
                    <input type="checkbox" 
                           name="ativo" 
                           id="ativo" 
                           <?php echo (!$demonstrativo || $demonstrativo['ativo']) ? 'checked' : ''; ?>
                    ...
                    <label for="ativo" class="ml-2 text-sm font-medium text-gray-700">
                        Demonstrativo ativo (visível no site)
                    </label>
                </div>
                ...
                <div class="flex gap-4 pt-6 border-t">
                    <button type="submit" 
                        ...
                        <?php echo $id ? 'Atualizar' : 'Criar'; ?> Demonstrativo
                    </button>
                    <a href="demonstrativos" 
