...
// Buscar card
$stmt = $pdo->prepare("SELECT * FROM financeiro_cards WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'update_card') {
        $stmt = $pdo->prepare("UPDATE financeiro_cards SET titulo = ?, descricao = ? WHERE id = ?");
        $stmt->execute([$_POST['titulo'], $_POST['descricao'], $card_id]);
        header("Location: editar-financeiro.php?id=$card_id&success=1");
        exit;
    }
    
    // === SEÇÃO ===
    if ($action === 'add_secao') {
        $stmt = $pdo->prepare("SELECT COALESCE(MAX(ordem), 0) + 1 as nova_ordem FROM financeiro_secoes WHERE card_id = ?");
        $stmt->execute([$card_id]);
        $nova_ordem = $stmt->fetch()['nova_ordem'];
        
        $stmt = $pdo->prepare("INSERT INTO financeiro_secoes (card_id, titulo, ordem) VALUES (?, ?, ?)");
    ...
    if ($action === 'update_secao') {
        $stmt = $pdo->prepare("UPDATE financeiro_secoes SET titulo = ? WHERE id = ?");
    ...
    if ($action === 'delete_secao') {
        $stmt = $pdo->prepare("DELETE FROM financeiro_secoes WHERE id = ?");
    ...
    if ($action === 'move_secao') {
        ...
        $stmt = $pdo->prepare("SELECT * FROM financeiro_secoes WHERE id = ?");
        $stmt->execute([$secao_id]);
        $secao = $stmt->fetch();
        
        if ($secao) {
            if ($direcao === 'up') {
                $stmt = $pdo->prepare("SELECT * FROM financeiro_secoes WHERE card_id = ? AND ordem < ? ORDER BY ordem DESC LIMIT 1");
            } else {
                $stmt = $pdo->prepare("SELECT * FROM financeiro_secoes WHERE card_id = ? AND ordem > ? ORDER BY ordem ASC LIMIT 1");
            }
            $stmt->execute([$card_id, $secao['ordem']]);
            $outra = $stmt->fetch();
            
            if ($outra) {
                $stmt = $pdo->prepare("UPDATE financeiro_secoes SET ordem = ? WHERE id = ?");
    ...
    if ($action === 'add_subsecao') {
        // Buscar maior ordem_geral na seção (considerando subseções e anos diretos)
        $stmt = $pdo->prepare("SELECT COALESCE(MAX(ordem_geral), 0) as max_ordem FROM financeiro_subsecoes WHERE secao_id = ?");
        $stmt->execute([$_POST['secao_id']]);
        $max_sub = $stmt->fetch()['max_ordem'];
        
        $stmt = $pdo->prepare("SELECT COALESCE(MAX(ordem_geral), 0) as max_ordem FROM financeiro_anos WHERE secao_id = ? AND (subsecao_id IS NULL OR subsecao_id = 0)");
        $stmt->execute([$_POST['secao_id']]);
        $max_ano = $stmt->fetch()['max_ordem'];
        
        $nova_ordem = max($max_sub, $max_ano) + 10;
        
        $stmt = $pdo->prepare("INSERT INTO financeiro_subsecoes (secao_id, titulo, ordem, ordem_geral) VALUES (?, ?, ?, ?)");
    ...
    if ($action === 'update_subsecao') {
        $stmt = $pdo->prepare("UPDATE financeiro_subsecoes SET titulo = ? WHERE id = ?");
    ...
    if ($action === 'delete_subsecao') {
        $stmt = $pdo->prepare("DELETE FROM financeiro_subsecoes WHERE id = ?");
        $stmt->execute([$_POST['subsecao_id']]);
        header("Location: editar-financeiro.php?id=$card_id&success=1");
        exit;
    }
    
    // === MOVER ITEM (subseção ou ano direto) - INTERCALADO ===
    if ($action === 'move_item') {
        $tipo = $_POST['tipo']; // 'subsecao' ou 'ano'
        ...
        // Buscar item atual
        if ($tipo === 'subsecao') {
            $stmt = $pdo->prepare("SELECT id, ordem_geral, 'subsecao' as tipo FROM financeiro_subsecoes WHERE id = ?");
        } else {
            $stmt = $pdo->prepare("SELECT id, ordem_geral, 'ano' as tipo FROM financeiro_anos WHERE id = ?");
        }
        $stmt->execute([$item_id]);
        $item = $stmt->fetch();
        
        if ($item) {
            // Buscar todos os itens da seção (subseções + anos diretos) ordenados
            $stmt = $pdo->prepare("
                SELECT id, ordem_geral, 'subsecao' as tipo FROM financeiro_subsecoes WHERE secao_id = ?
                UNION ALL
                SELECT id, ordem_geral, 'ano' as tipo FROM financeiro_anos WHERE secao_id = ? AND (subsecao_id IS NULL OR subsecao_id = 0)
            ...
            // Trocar ordem_geral com o vizinho
            if ($vizinho) {
                if ($tipo === 'subsecao') {
                    $stmt = $pdo->prepare("UPDATE financeiro_subsecoes SET ordem_geral = ? WHERE id = ?");
                } else {
                    $stmt = $pdo->prepare("UPDATE financeiro_anos SET ordem_geral = ? WHERE id = ?");
                }
    ...
    if ($action === 'add_ano') {
        ...
        // Se é ano direto (sem subseção), calcular ordem_geral
        $ordem_geral = 0;
        ...
        $stmt = $pdo->prepare("INSERT INTO financeiro_anos (secao_id, subsecao_id, ano, ordem_geral) VALUES (?, ?, ?, ?)");
    ...
    if ($action === 'update_ano') {
        $stmt = $pdo->prepare("UPDATE financeiro_anos SET ano = ? WHERE id = ?");
    ...
    if ($action === 'delete_ano') {
        $stmt = $pdo->prepare("DELETE FROM financeiro_anos WHERE id = ?");
        $stmt->execute([$_POST['ano_id']]);
        header("Location: editar-financeiro.php?id=$card_id&success=1");
        exit;
    }
    
    // move_ano continua existindo para anos DENTRO de subseções
    if ($action === 'move_ano') {
        $ano_id = $_POST['ano_id'];
        $direcao = $_POST['direcao'];
        
        $stmt = $pdo->prepare("SELECT * FROM financeiro_anos WHERE id = ?");
        $stmt->execute([$ano_id]);
        $ano = $stmt->fetch();
        
        if ($ano && $ano['subsecao_id']) {
            // Anos dentro de subseção - ordena por ano DESC
            if ($direcao === 'up') {
                $stmt = $pdo->prepare("SELECT * FROM financeiro_anos WHERE subsecao_id = ? AND ano > ? ORDER BY ano ASC LIMIT 1");
            } else {
                $stmt = $pdo->prepare("SELECT * FROM financeiro_anos WHERE subsecao_id = ? AND ano < ? ORDER BY ano DESC LIMIT 1");
            }
    ...
    if ($action === 'add_arquivo') {
        ...
        // Título: ou mês selecionado ou personalizado
        ...
        if (move_uploaded_file($file['tmp_name'], $file_path)) {
            $stmt = $pdo->prepare("INSERT INTO financeiro_arquivos (ano_id, titulo, arquivo_path) VALUES (?, ?, ?)");
            $stmt->execute([$_POST['ano_id'], $titulo, '/uploads/financeiro/' . $file_name]);
        }
        header("Location: editar-financeiro.php?id=$card_id&success=1");
        exit;
    }
    ...
    if ($action === 'update_arquivo') {
        $stmt = $pdo->prepare("UPDATE financeiro_arquivos SET titulo = ? WHERE id = ?");
    ...
    if ($action === 'delete_arquivo') {
        $stmt = $pdo->prepare("SELECT arquivo_path FROM financeiro_arquivos WHERE id = ?");
        ...
        $stmt = $pdo->prepare("DELETE FROM financeiro_arquivos WHERE id = ?");
        $stmt->execute([$_POST['arquivo_id']]);
        header("Location: editar-financeiro.php?id=$card_id&success=1");
        exit;
    }
...
<div class="max-w-5xl mx-auto space-y-6">
    <div class="mb-8">
        <a href="financeiro.php" class="text-blue-600 hover:underline mb-4 inline-block">← Voltar para Financeiro</a>
    ...
    <?php
        ...
        // Buscar anos diretos da seção (sem subseção) com tipo
        ...
        foreach ($anos_secao as $a) {
            $st = $pdo->prepare("SELECT COUNT(*) FROM financeiro_arquivos WHERE ano_id = ?");
            $st->execute([$a['id']]);
            $totalSecao += $st->fetchColumn();
        }
        foreach ($subsecoes as $sub) {
            $st = $pdo->prepare("SELECT fa.id FROM financeiro_arquivos fa JOIN financeiro_anos an ON fa.ano_id = an.id WHERE an.subsecao_id = ?");
            $st->execute([$sub['id']]);
    ...
    <div class="bg-white rounded-xl shadow-md p-6">
        <!-- Cabeçalho da Seção -->
        <div class="flex items-center justify-between mb-4 pb-4 border-b-2 border-green-500">
        ...
        <!-- Adicionar Subseção ou Ano na Seção -->
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            ...
            <div class="bg-green-50 rounded-lg p-4 border border-green-200">
                <h3 class="text-sm font-semibold text-green-700 mb-3">📅 Novo Ano (direto na seção)</h3>
                <form method="POST" class="flex gap-2">
                    <input type="hidden" name="action" value="add_ano">
                    <input type="hidden" name="secao_id" value="<?= $secao['id'] ?>">
                    <input type="number" name="ano" placeholder="Ex: 2025" required min="2000" max="2100" class="w-28 border border-green-300 rounded px-3 py-2 text-sm">
                    <button type="submit" class="bg-green-500 text-white px-4 py-2 rounded text-sm hover:bg-green-600">+ Ano</button>
                </form>
            </div>
        </div>

        <!-- Itens da Seção (subseções e anos intercalados) -->
        <?php foreach ($itens_secao as $item_idx => $item): ?>
            <?php if ($item['tipo'] === 'subsecao'): ?>
                ...
                <?php
                    ...
                    $totalSub = 0;
                    foreach ($anos_sub as $a) {
                        $st = $pdo->prepare("SELECT COUNT(*) FROM financeiro_arquivos WHERE ano_id = ?");
                        $st->execute([$a['id']]);
                        $totalSub += $st->fetchColumn();
                    }
                ?>
                <div class="bg-purple-50 rounded-lg p-4 mb-4 border-l-4 border-purple-400">
                    ...
                    <?php include __DIR__ . '/includes/_ano_bloco.php'; ?>
                    <?php endforeach; ?>
                </div>
            <?php else: ?>
                <?php 
                    ...
                    $ano_idx = $item_idx;
                ?>
                <div class="mb-4 border-l-4 border-green-400 rounded-lg overflow-hidden">
                    <div class="flex items-center gap-1 bg-green-50 px-2 py-1">
                        <?php if ($item_idx > 0): ?>
                        <form method="POST" class="inline"><input type="hidden" name="action" value="move_item"><input type="hidden" name="tipo" value="ano"><input type="hidden" name="item_id" value="<?= $ano['id'] ?>"><input type="hidden" name="secao_id" value="<?= $secao['id'] ?>"><input type="hidden" name="direcao" value="up">
                            <button class="p-1 text-gray-500 hover:text-green-600 text-xs" title="Mover para cima">▲</button>
                        </form>
                        <?php endif; ?>
                        <?php if ($item_idx < count($itens_secao) - 1): ?>
                        <form method="POST" class="inline"><input type="hidden" name="action" value="move_item"><input type="hidden" name="tipo" value="ano"><input type="hidden" name="item_id" value="<?= $ano['id'] ?>"><input type="hidden" name="secao_id" value="<?= $secao['id'] ?>"><input type="hidden" name="direcao" value="down">
                            <button class="p-1 text-gray-500 hover:text-green-600 text-xs" title="Mover para baixo">▼</button>
                        </form>
                        <?php endif; ?>
                    </div>
                    <?php include __DIR__ . '/includes/_ano_bloco.php'; ?>
    ...
    <div class="bg-white rounded-xl p-6 w-full max-w-md mx-4">
        ...
        <form method="POST" enctype="multipart/form-data" class="space-y-4">
            ...
            <div class="flex gap-3">
                <button type="submit" class="flex-1 bg-green-primary text-white px-4 py-2 rounded-lg font-semibold hover:bg-green-700 transition-colors">Enviar</button>
...
<div id="modalEditDocumento" class="fixed inset-0 bg-black bg-opacity-50 hidden z-50 flex items-center justify-center">
    <div class="bg-white rounded-xl p-6 w-full max-w-md mx-4">
        ...
        <form method="POST" class="space-y-4">
            ...
            <div class="flex gap-3">
                <button type="submit" class="flex-1 bg-green-primary text-white px-4 py-2 rounded-lg font-semibold hover:bg-green-700 transition-colors">Salvar</button>
...
function editarInline(tipo, id, valorAtual) {
    ...
    form.innerHTML = `
        ...
        <button type="submit" class="bg-green-500 text-white px-2 py-1 rounded text-xs">OK</button>
