...
// Buscar categoria
$stmt = $pdo->prepare("SELECT * FROM categorias_balancetes WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] === 'add_ano') {
        $ano = (int)$_POST['ano'];
        header("Location: editar-balancete.php?categoria=$categoria_id&success=ano_ok&novo_ano=$ano");
        exit;
    }
    
    // Adicionar Arquivo/Balancete
    if ($_POST['action'] === 'add_arquivo') {
        ...
        if (empty($erros)) {
            $stmt = $pdo->prepare("SELECT COALESCE(MAX(ordem), 0) + 1 as proxima FROM balancetes WHERE categoria_id = ? AND ano = ?");
            $stmt->execute([$categoria_id, $ano]);
            $ordem = $stmt->fetchColumn();
            
            $stmt = $pdo->prepare("INSERT INTO balancetes (categoria_id, titulo, arquivo, data_documento, ano, ordem, ativo, criado_em) VALUES (?, ?, ?, ?, ?, ?, 1, NOW())");
            $stmt->execute([$categoria_id, $titulo, $arquivo_path, date('Y-m-d'), $ano, $ordem]);
            
            header("Location: editar-balancete.php?categoria=$categoria_id&success=arquivo_added");
            exit;
        } else {
            header("Location: editar-balancete.php?categoria=$categoria_id&error=" . urlencode(implode(' ', $erros)));
            exit;
        }
    }
    ...
    if ($_POST['action'] === 'delete_arquivo') {
        ...
        $stmt = $pdo->prepare("SELECT arquivo FROM balancetes WHERE id = ? AND categoria_id = ?");
        ...
        $stmt = $pdo->prepare("DELETE FROM balancetes WHERE id = ? AND categoria_id = ?");
        $stmt->execute([$arquivo_id, $categoria_id]);
        
        header("Location: editar-balancete.php?categoria=$categoria_id&success=arquivo_deleted");
        exit;
    }
    ...
    if ($_POST['action'] === 'delete_ano') {
        ...
        $stmt = $pdo->prepare("SELECT arquivo FROM balancetes WHERE categoria_id = ? AND ano = ?");
        $stmt->execute([$categoria_id, $ano]);
        ...
        $stmt = $pdo->prepare("DELETE FROM balancetes WHERE categoria_id = ? AND ano = ?");
        $stmt->execute([$categoria_id, $ano]);
        
        header("Location: editar-balancete.php?categoria=$categoria_id&success=ano_deleted");
        exit;
    }
    ...
    if ($_POST['action'] === 'update_ano') {
        ...
        $stmt = $pdo->prepare("UPDATE balancetes SET ano = ? WHERE id = ? AND categoria_id = ?");
        $stmt->execute([$novo_ano, $ano_id, $categoria_id]);
        
        header("Location: editar-balancete.php?categoria=$categoria_id&success=ano_updated");
        exit;
    }
    ...
    if ($_POST['action'] === 'move_arquivo') {
        ...
        $stmt = $pdo->prepare("SELECT ano, ordem FROM balancetes WHERE id = ? AND categoria_id = ?");
        $stmt->execute([$arquivo_id, $categoria_id]);
        ...
        if ($arquivo_atual) {
            ...
            if ($direcao === 'up') {
                ...
                $stmt = $pdo->prepare("SELECT id, ordem FROM balancetes WHERE categoria_id = ? AND ano = ? AND ordem < ? ORDER BY ordem DESC LIMIT 1");
                $stmt->execute([$categoria_id, $ano, $ordem_atual]);
            } else {
                // Trocar com o próximo arquivo
                $stmt = $pdo->prepare("SELECT id, ordem FROM balancetes WHERE categoria_id = ? AND ano = ? AND ordem > ? ORDER BY ordem ASC LIMIT 1");
                $stmt->execute([$categoria_id, $ano, $ordem_atual]);
            }
            ...
            if ($arquivo_troca) {
                ...
                $stmt = $pdo->prepare("UPDATE balancetes SET ordem = ? WHERE id = ?");
        ...
        header("Location: editar-balancete.php?categoria=$categoria_id&success=ordem_updated");
        exit;
    }
}

// Buscar todos os anos distintos para esta categoria
$stmt = $pdo->prepare("SELECT DISTINCT ano FROM balancetes WHERE categoria_id = ? ORDER BY ano DESC");
$stmt->execute([$categoria_id]);
...
<div class="container mx-auto px-4 py-8">
    <div class="mb-8">
        <a href="balancetes_index.php" class="text-blue-600 hover:underline mb-4 inline-block">← Voltar para Balancetes</a>
    ...
    foreach ($anos as $ano): 
        ...
        $stmt = $pdo->prepare("SELECT * FROM balancetes WHERE categoria_id = ? AND ano = ? ORDER BY ordem ASC");
        $stmt->execute([$categoria_id, $ano]);
        $arquivos = $stmt->fetchAll();
        $ano_index++;
    ?>
...
function moverAno(ano, direcao) {
    ...
    if (direcao === 'up' && index > 0) {
        window.location.href = `editar-balancete.php?categoria=<?php echo $categoria_id; ?>&move_ano=${ano}&dir=up`;
    } else if (direcao === 'down' && index < anos.length - 1) {
        window.location.href = `editar-balancete.php?categoria=<?php echo $categoria_id; ?>&move_ano=${ano}&dir=down`;
    }
}
</script>
